//
//  EmojiArtView+Extension(Gestures).swift
//  EmojiArt
//
//  Created by Саша Восколович on 08.09.2023.
//
...
extension EmojiArtView
{
    func addEmojiArtGestureRecognizers(to view: UIView) {
        ...
        view.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.selectSubview)))
        view.addGestureRecognizer(UIPanGestureRecognizer(target: self, action: #selector(self.selectAndMoveSubview)))
    }
    ...
    func enableRecognizers() {
        ...
        if gestureRecognizers == nil || gestureRecognizers!.count == 0 {
            ...
            addGestureRecognizer(UIPinchGestureRecognizer(target: self, action: #selector(self.resizeSelectedLabel)))
        } else {
